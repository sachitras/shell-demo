...
public class SSHCommand {
	...
	public void createTenant(@ShellOption(value = { "name" }, help = "Tenant name") String tenantName)  {
		try {
	...
	public void createProduct(@ShellOption(value = { "name" }, help = "Product family name") String productFamilyName,
							  @ShellOption(value = { "tenant" }, help = "Tenant Id") String tenantId,
							  @ShellOption(value = { "smart-contract" }, help = "Smart Contract URL", defaultValue = "") String smartContract)  {
		try {
...
//				}
	...
	@ShellMethod(key = { "px product-family configure"})
	public void configureProductFamily(@ShellOption(value = { "" }, help = "Product family id") String productFamilyId,
		...
		try {
			...
			if (productFamilyId != null) {
				...
				if (productFamilyDTO != null) {
					if (smartContract != null) {
						System.out.println("Configuring product family...");
						delay(4000);
						productFamilyDTO.setSmartContractName(smartContract);
						boolean flag = shellService.saveProductFamily(productFamilyDTO);
						if (flag) {
							System.out.println("Product family configured successfully.");
							System.out.println("-------------------------------------------------------");
							...
							System.out.println("Smart Contract      :" + productFamilyDTO.getSmartContractName());
						}
					} else {
						System.out.println("Please enter a smart contract url.");
					}

				} else {
					System.out.println("Product family not found. Please enter a valid product family id.");
				}

			} else {
				System.out.println("Please enter a product family.");
			}
	...
	public void addCapability(@ShellOption(value = { "select" }, help = "Capabilities separated by commas") String capabilities,
							  @ShellOption(value = { "product-family" }, help = "Product family id") String productFamilyId) {
		try {
	...
	public void provisionProductFamily(@ShellOption(value = { "product-family" }, help = "Product Family Id") String productFamilyId,
							  @ShellOption(value = { "env" }, help = "Environment") String env) {
		try {
	...
	public void configureCapabilityAPIExt(@ShellOption(value = { "" }, help = "Capability Name") String capabilityName,
										  @ShellOption(value = { "product-family" }, help = "Product Family Id") String productFamilyId,
									    @ShellOption(value = { "api-ext" }, help = "API Extension URL") String apiExt,
										  @ShellOption(value = { "adaptor-ext" }, help = "Adaptor Extension URL") String adaptorExt) {
		try {
	...
	public void listProductFamilyForTenantId(@ShellOption(value = { "tenant" }, help = "Tenant Id") String tenantId) {
		try {
	...
	public void describeTenant(@ShellOption(value = { "" }, help = "Tenant Id") String tenantId) {
		try {
			if (tenantId != null && tenantId.length() > 0) {
	...
	public void describeProductFamily(@ShellOption(value = { "" }, help = "Product Family Id") String productFamilyId) {
		try {
			if (productFamilyId != null && productFamilyId.length() > 0) {
	...
	public void describeCapabilities(@ShellOption(value = { "" }, help = "Capability Name") String capabilityName,
									 @ShellOption(value = { "product-family-id" }, help = "Product Family Id") String productFamilyId) {
		try {
			if (productFamilyId != null && productFamilyId.length() > 0) {
